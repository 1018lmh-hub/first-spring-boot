...
public class AuthService {
	...
	public LoginResponse login(LoginRequestDto lrd) {
		...
		// 1. 유효성검증(아이디 / 비밀번호 값이 들어왔는가, 영어숫자인가, 글자수가 괜찮은가) -> @Valid로 대체 
		// 2. 아이디가 SEMI_MEMBER 테이블에 MEMBER_ID 컬럼에 존재하는 아이디인가? -> loadUserByUserName
		// 3. 조회를 해온 비밀번호 컬럼에 암호문이 사용자가 입력한 평문으로 만들어진 것이 맞는가? -> Provider가 해줌
		...
		try {
			...
			throw new CustomAuthenticationException("아이디 또는 비밀번호가 이상혀");
		}		
